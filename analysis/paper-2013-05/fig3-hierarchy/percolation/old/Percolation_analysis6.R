...
PERCOL.DIR <- paste0(FIGURES.ROOT.DIR, "Fig2-Hierarchical/Percolation/")
...
contact.sheet <- function(edgelist.file, outfile="") {
  ...
  percolation.figure(edgelist.file, "", convert.to.undirected=F, min.weight=100, min.exposure=0.01, max.pval=0.001, 
                       deg.type="total",user.langs.to.display=50, user.langs.to.label=15)  
  percolation.figure(edgelist.file, "", convert.to.undirected=F, min.weight=100, min.exposure=0.01, max.pval=0.001, 
                       deg.type="total", comp.conn.mode="weak",user.langs.to.display=50, user.langs.to.label=15)  
  if (outfile!="") {
...
main <- function() {
  ...
  op <- par(mfrow=c(3,4), mar=c(4,4,4,4), oma=c(0,4,0.5,4), pty="s")
  ...
  contact.sheet(BOOKS.IN) # only total degree
  if (outfile!="") {
    dev.off()
  }
  par(op)
