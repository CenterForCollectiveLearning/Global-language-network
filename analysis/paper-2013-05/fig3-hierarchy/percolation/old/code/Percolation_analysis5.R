...
PERCOL.DIR <- paste0(FIGURES.ROOT.DIR, "Fig3-Hierarchical/Percolation/")
...
COMPONENT.CONNECTIVITY.MODE <- "weak" # weak or strong
...
percolation.figure <- function(infile, 
                               ...
                               comp.conn.mode="strong", # are we looking for strongly or weakly connected components
  ...
  title.text <- sprintf("%s/%s degree/undir=%s\n%s\nexpo>=%s/pval<%sconn=%s", 
                        ...
                        min.weight, min.exposure, max.pval,comp.conn.mode)
...
contact.sheet <- function(edgelist.file, outfile="", is.books=F) {
  ...
  if (is.books==F) {
#     percolation.figure(edgelist.file, "", min.weight=100,
#                        deg.type="total", user.langs.to.display=50, user.langs.to.label=15)
    percolation.figure(edgelist.file, "", min.weight=100, min.exposure=0.01, max.pval=0.001, 
                       deg.type="total", user.langs.to.display=50, user.langs.to.label=15)
    percolation.figure(edgelist.file, "", min.weight=100, min.exposure=0.01, max.pval=0.001, 
                       deg.type="total", comp.conn.mode="weak",user.langs.to.display=50, user.langs.to.label=15)  
  }  
  else {
...
#                        deg.type="out", user.langs.to.display=50, user.langs.to.label=15)
    percolation.figure(edgelist.file, "", min.weight=25, min.exposure=0.01, max.pval=0.001,
                       deg.type="out", user.langs.to.display=50, user.langs.to.label=15)
    percolation.figure(edgelist.file, "", min.weight=25, min.exposure=0.01, max.pval=0.001,
                       deg.type="out", comp.conn.mode="weak", user.langs.to.display=50, user.langs.to.label=15)
  }
  if (outfile!="") {
    dev.off()
  }
...
main <- function() {
  ...
  op <- par(mfrow=c(3,2), mar=c(4,4,4,4), oma=c(0,4,0.5,4), pty="s")
  ...
  contact.sheet(BOOKS.IN, is.books=F) # only total degree
  if (outfile!="") {
    dev.off()
  }
  par(op)
