...
prep.lgn.df <- function(twitter.edge.file, wiki.edge.file, books.edge.file,
  ...
  # Merge the tables
  tmp <- merge(twitter.metrics, wiki.metrics, by="language", all=ALLOW.UNCOMMON.LANGS)
  rownames(tmp) <- tmp$Row.names # Row.names is created unwillingly, rename...
  tmp$Row.names <- NULL # ...and remove
  graph.metrics <- merge(tmp, books.metrics,  by="language", all=ALLOW.UNCOMMON.LANGS)
  rownames(graph.metrics) <- graph.metrics$Row.names # Repeat...
  graph.metrics$Row.names <- NULL
  
  
  # Rename the table
  all.metrics <- graph.metrics
  names(all.metrics) <- c("language", "twit.eig", "twit.popfrom", "twit.popto",
                          "wiki.eig", "wiki.popfrom", "wiki.popto",
                          "book.eig", "book.popfrom", "book.popto")
  rownames(all.metrics) <- all.metrics$language
  rm(graph.metrics)
  rm(tmp)
