...
DEGREE.DIR <- paste0(ANALYSIS.ROOT.DIR, "/fig3-hierarchy-null-model/degree/")

OUTPUT.FILE <- "figure3_users500_trans300_exp0.001_degreedist_v35.eps"
...
degree.distribution.figure <- function(lgn.graph, 
  ...
  plot(bins1$mids, probs, 
       ...
       main=fig.title, pch=16, cex=1, ylab=y.label)
...
degree.figure <- function(infile, 
  ...
  # print avg. degree
  print(sprintf("### Avg. degree: %s", round(mean(lgn.metrics$deg),2)))
  ...
  if(plot.rank==TRUE) {
      ...
      plot(c(1:num.langs.to.display), 
           ...
           main=sprintf("%s (av. deg. %s)", title.text, 
                        round(mean(lgn.metrics$deg),2)),
