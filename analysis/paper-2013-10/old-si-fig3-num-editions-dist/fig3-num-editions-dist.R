...
DB.PATH <- file.path(WIKI.PRODUCTION.DIR, "wiki_data/l26_observatory_manualcleaning3.sqlite3")
...
people.table <- dbReadTable(conn, "culture3")
...
notability.plot <- ggplot(notability.dist, aes(x=numlangs, y=cum.people.per.num)) +
  ...
  scale_x_continuous(breaks=c(0,THRES,50,100,150,200)) + 
  scale_y_log10(breaks=c(1e+01, 1e+02, 1e+03, PEOPLE.OVER.THRES ,1e+05),
                labels=c("1e+01", "1e+02", "1e+03", PEOPLE.OVER.THRES ,"1e+05")) +
