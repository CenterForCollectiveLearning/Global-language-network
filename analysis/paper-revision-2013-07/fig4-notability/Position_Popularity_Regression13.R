...
prep.lgn.df <- function(twitter.edge.file, wiki.edge.file, books.edge.file,
  ...
  # Merge centrality measure from the different sources, and add population
  ...
  all.metrics <- merge(tmp, books.metrics, by=0, all=ALLOW.UNCOMMON.LANGS)
  rownames(all.metrics) <- all.metrics$Row.names # Repeat...
  all.metrics$Row.names <- NULL #

  # Add GDP per capita and cultural exports
  lang.exports <- read.table(cultural.exports.file, sep = "\t", header=T)
  rownames(lang.exports) <- lang.exports$lang_code
  all.metrics$gdp.pc <-
    lang.exports[match(rownames(all.metrics),
                          rownames(lang.exports)),"gdp_per_capita"]# For some reason GDP stats were multiplied by 100k, this fixes it.  TODO: remove fix if data are corrected.
  all.metrics$cultexp <-
    lang.exports[match(rownames(all.metrics),
                      rownames(lang.exports)),"total_exports"]
  
  all.metrics.orig <- all.metrics
  # Remove languages without listed cultural exports
  all.metrics <- subset(all.metrics, !is.na(cultexp))
  
  # Remove langs with fewer than min.exports cultural exps
  ...
  # Add number of speakers to each language
  speakers.families <- read.csv(speaker.stats.file, sep = "\t", header=T)
  rownames(speakers.families) <- speakers.families$Lang_Code
  all.metrics$pop <-
    speakers.families[match(rownames(all.metrics),
                          rownames(speakers.families)),"Num_Speakers_M"]
...
regress.one.indep <- function(plot.vars,
  ...
  ggp <- ggplot(plot.vars, aes(x=indeps, y=deps, label=labels)) + 
    #geom_point(aes(size=sizes+10), color="black") + # data point line color
    ...
    scale_size(
      ...
      range=c(4, 15), name="Millions of Speakers") + 
    scale_color_gradient(limits=c(1000,60000), # Standardize GDPpc scale across plots: 1k to 60k
                         low="grey91", high=plot.color, name="GDP per Capita") + 
    xlim(c(0,1)) + 
    #ylim(c(0,3)) +
    labs(list(x=x.title, y=y.title, title=src.name)) +
    geom_text(aes(0.7, 0.5, label=sprintf("R\UB2 =%s\np-value < %s",
                                             round(r.squared, 3),
                                             round(p.val.cat, 3))),
...
prep.plot.vars <- function(df.all.metrics,
  ...
  plot.vars <- with(df.all.metrics, 
                    data.frame(
                      ...
                      deps=log10(get(dep)+LOG.SMOOTH.ADD), # dep. variable smooth zeros
                      sizes=pop,     # data point size is population
                      colors=gdp.pc, # data point color is GDP pc
...
plot.regressions <- function(df.all.metrics, outfile="") {
  ...
  p.book <- regress.one.indep(plot.vars.book,
                    src.name="Books",
                    x.title="Books EV cent.", y.title="log10(Popularity)",
...
regression.table.multi.source <- function(all.metrics,
                                          src.name,
                                          outfile="") #
{
  ...
  # Log vectors with uneven distribution
  pop.reg <- log10(all.metrics$pop) # not smoothed so we're notified of zeros
  gdp.reg <- log10(all.metrics$gdp)
  cultexp.reg <- log10(all.metrics$cultexp+LOG.SMOOTH.ADD) # smooth zeros

  lm1 <<- lm(cultexp.reg ~ pop.reg + gdp.reg)
  lm2 <<- lm(cultexp.reg ~ all.metrics$twit.eig)
  lm3 <<- lm(cultexp.reg ~ all.metrics$wiki.eig)
  lm4 <<- lm(cultexp.reg ~ all.metrics$book.eig)
  lm5 <<- lm(cultexp.reg ~ pop.reg + gdp.reg + all.metrics$twit.eig)
  lm6 <<- lm(cultexp.reg ~ pop.reg + gdp.reg + all.metrics$wiki.eig)
  lm7 <<- lm(cultexp.reg ~ pop.reg + gdp.reg + all.metrics$book.eig)
...
run.notability.regressions <- function(src.name, # "wiki" / "murray"
  ...
  #scenario.settings <- sprintf("EV_%s_twcomm%s_bcomm%s_expo%s_pval%s_ppl%s",
  #                             SRC.NAME, MIN.COMMON.USERS, MIN.COMMON.TRANS, MIN.EXPOSURE,
  #                             DESIRED.P.VAL, MIN.EXPORTS)
  ...
  all.metrics <- prep.lgn.df(TWIT.STD.LANGLANG, 
                             WIKI.STD.LANGLANG, 
                             BOOKS.STD.LANGLANG,
                             SPEAKER.STATS.FILE,
  ...
  # Draw the plots.
  plot.regressions(all.metrics, outfile)
  
  # Create a regression table for all sources
  regression.table.multi.source(all.metrics, src.name=src.name, outfile=outfile)
  ...
  # Strucutre of Wiki: countryName  countryCode  numppl
  # Structure of Murray: "WorkCountry  Total" or "BirthCountry  Total"
  
  # The following hack standardizes columns across Murray and Wiki tables.
  # Both will have "countryCode  numppl". We set countryCod to NA,
  # because R doesn't like subsetting single-column dataframes
  #print(country.exports[1:10,]) # Number of exports per country
  country.exports$countryCode <- NULL
  colnames(country.exports)[1] <- "numppl"
  country.exports["countryCode"] <- NA
  
  # Now write only the country name and number of people columns
  write.table( subset(country.exports[order(-country.exports[1])[1:11],], select=c(numppl)),
               file=top10.file, append=T, quote=F, sep="\t")
...
run.notability.regressions(src.name="murray", 
                           date.range="1800_1950",
                           add.note="nosqi",
                           langs.to.remove=c("sqi")) # Albanian is a major outlier
#run.notability.regressions(src.name="murray", 
#                           date.range="all",
#                           add.note="nosqi",
