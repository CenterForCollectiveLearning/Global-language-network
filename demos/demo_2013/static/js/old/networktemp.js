...
function updateForce(minThreshold, maxThreshold) {
    ...
    //force.nodes(nodeList.filter( function( d ) { return newNodes[d.name];}));
    //force.links(links.filter( function( d ) { return d.influence>threshold;}));
...
//    newLinkList.forEach( function(d) { alert(languageData[d.source.name].fullname+" "+languageData[d.target.name].fullname);});
//    newNodeList.forEach( function(d) { alert(languageData[d.name].fullname);});
...
function selectMeme(meme) {
    var setupDelay = 3000;       //time before animation starts
...
function tick() {
    ...
    $('#slider').slider({
        ...
        slide: function( event, ui ) {
            ...
            //alert(min_inf + " " + max_inf);
            //updateForce(.015,1.1);
            updateForce(min_inf,max_inf);
        }
    });
    
    // TODO: uncomment for diffusion
